...
logout_schema = openapi.Schema(
    type=openapi.TYPE_OBJECT,
    properties={
        'message': openapi.Schema(type=openapi.TYPE_STRING, description="Mensaje indicando que la sesión fue cerrada exitosamente"),
    },
)

# Esquema de respuesta para error de token
error_token_schema = openapi.Schema(
    type=openapi.TYPE_OBJECT,
    properties={
        'error': openapi.Schema(type=openapi.TYPE_STRING, description="Mensaje de error en caso de que el token no sea válido o no esté presente"),
    },
)
...
@swagger_auto_schema(
    method='post',
    responses={
        200: logout_schema,  # Respuesta exitosa con el mensaje de cierre de sesión
        401: error_token_schema,  # Respuesta para token no proporcionado o inválido
        500: openapi.Response(description="Error interno del servidor")  # Respuesta para error inesperado
    }
)
